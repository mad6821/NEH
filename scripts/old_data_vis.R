...
# Set directories (may have to set own path file on personal device)
out_dir <- "~/Desktop/NEH/Project/MD_InternProject"
neh_dir <- file.path(out_dir, "NEHData")
ipeds_dir <- file.path(out_dir, "IPEDSData")
econ_dir <- file.path(out_dir, "EconData")

# Load datasets
econ_df <- read_csv(file.path(econ_dir, "econ_df.csv")) # Census economic data
arc_clean <- read_csv(file.path(out_dir, "arc_clean.csv")) # ARC Data for counties in Appalachia
arc_clean <- arc_clean %>%
  mutate_if(is.character, str_trim) # Trim whitespace off names

df_clean <- read_csv(file.path(out_dir, "df_clean.csv")) # Main data frame
df_clean <- df_clean %>%
  mutate_if(is.character, str_trim) # Trim whitespace off names
...
# Subsetting Econ and sf data to avoid counties with the same name in different states 
# overlapping one another (i.e., Jefferson Co. in AL, OH, and WV)

out_list <- purrr::map(app_states,~ {
  econ_state <- subset(econ_df, State == tools::toTitleCase(.x)) # Subset econ data by state
  # Subset sf counties data by counties in econ data 
  counties_state <- subset(counties, county %in% tolower(econ_state$County) & state == .x)  
  return(counties_state)                   
})
  
counties_app <- bind_rows(out_list) # turn list into single df
...
award_table <- df_clean %>%
  mutate(total_award = ifelse(OriginalAmount == 0, AwardOutright, OriginalAmount),
